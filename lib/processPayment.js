...
class Payment {
  ...
  async isEnoughAmount(payment) {
    ...
    let earned = ss.amount.aht.earned || 0
    let paid = ss.amount.aht.paid || 0
    let tip = ss.amount.aht.tip || 0
    let tipped = ss.amount.aht.tipped || 0
  ...
  async addAmount(payment, tx_id, sent_at) {
    ...
    let divider = 100000000
    amount.aht.paid =
      Math.round((amount.aht.paid + payment.val.amount) * divider) / divider
    await firestore
      .collection("users_server")
      .doc(payment.val.uid)
  ...
  async sendToken(payment) {
    ...
    const transferData = {
      ...
      feeAssetId: "WAVES",
      amount: Math.round(payment.amount * 10 ** process.env.ASSET_PRECISION),
      fee: FEE,
      timestamp: sent_at
    }
