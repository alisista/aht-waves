  ...
  async updateTips() {
    ...
    for (let tip of tips) {
      ...
      if (ss2.exists) {
        ...
        let tip_others = article.tip_others || {}
        if (tip.data.asset == undefined || tip.data.asset === "AHT") {
          article_tip += tip.data.amount
          article_tip = this.rmFraction(article_tip)
        } else {
          if (tip_others[tip.data.asset] == undefined) {
            tip_others[tip.data.asset] = {
              name: tip.data.asset_name,
              amount: 0
            }
          }
          tip_others[tip.data.asset].amount += tip.data.amount
          tip_others[tip.data.asset].amount = this.rmFraction(
            tip_others[tip.data.asset].amount
          )
        }
        ...
        await firestore
          ...
          .update({
            tip: article_tip,
            last_tip: tip.data.date,
            tip_others: tip_others
          })
        await firestore
          .collection("tip_pool")
          .doc(tip.id)
